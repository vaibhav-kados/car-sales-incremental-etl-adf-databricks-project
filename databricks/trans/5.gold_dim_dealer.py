...
df_filter_new_with_key = df_filter_new.withColumn(
    'dim_dealer_key',
    max_value + F.monotonically_increasing_id()
)

# COMMAND ----------

df_filter_new_with_key.display()
...
df_final = df_filter_new_with_key.union(df_filter_old)
...
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer'):
    delta_table = DeltaTable.forPath(spark, "abfss://gold@adlscarsalesp.dfs.core.windows.net/dim_dealer")
...
# Initial RUN 
    ...
    df_final.write.format("delta")\
        .mode("overwrite")\
        .option("path", "abfss://gold@adlscarsalesp.dfs.core.windows.net/dim_dealer")\
